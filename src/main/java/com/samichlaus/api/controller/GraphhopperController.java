...
public class GraphhopperController {
  ...
  public ResponseEntity<RouteTime> calculateRoute(@Valid @RequestBody RouteTime routeTime) {
    ...
    timeInterval =
        Math.round(
                (float)
                        graphhopperService.calculateTime(
                            depotLatLng.get(0),
                            depotLatLng.get(1),
                            currentCustomer.getAddress().getLatitude(),
                            currentCustomer.getAddress().getLongitude(),
                            routeTime.getTransport().toString())
                    / 1000
                    / 60
                    / 5)
            * 5;
          ...
          Math.round(
                  (float)
                          graphhopperService.calculateTime(
                              currentCustomer.getAddress().getLatitude(),
                              currentCustomer.getAddress().getLongitude(),
                              currentCustomerPlusOne.getAddress().getLatitude(),
                              currentCustomerPlusOne.getAddress().getLongitude(),
                              currentCustomer.getTransport().toString())
                      / 1000
                      / 60
                      / 5)
              * 5;
      timeInterval +=
          Constants.getChildrenSeniorCapacity(
              currentCustomer.getChildren(), currentCustomer.getSeniors());
      currentTime = currentTime.plusMinutes(timeInterval);
