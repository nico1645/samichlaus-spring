...
public class VRPService {
    ...
    public VehicleRoutingProblem createVehicleRoutingProblem(List<VRPVisit> vrpVisits, Integer maxVehiclesPerGroup, Integer maxGroups, Integer maxVisitTime) {
        VRPTransportCost vrpTransportCost = new VRPTransportCost(graphhopperService.calculateTimeMatrix(vrpVisits));
        ...
        for (int i=0; i < maxGroups; i++) {
            ...
            VehicleTypeImpl.Builder vehicleTypeBuilder = VehicleTypeImpl.Builder.newInstance("groupTransportType").addCapacityDimension(WEIGHT_INDEX, maxVehiclesPerGroup).addCapacityDimension(WEIGHT_VISIT_INDEX, maxVisitTime).setProfile("foot");
    ...
    private void addVisitJobs(VehicleRoutingProblem.Builder vrpBuilder, List<VRPVisit> customers) {
        for (int i = 0; i < customers.size(); i++) {
            ...
            vrpBuilder.addJob(Service.Builder.newInstance(i + "").addSizeDimension(WEIGHT_INDEX, 1).addSizeDimension(WEIGHT_VISIT_INDEX, vrpVisit.getCapacity()).setLocation(Location.newInstance(i + 1)).build());
        }
    }
