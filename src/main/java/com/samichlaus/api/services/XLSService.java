    ...
    for (int j = 0; j < 3; j++) {
      ...
      if (tourOpt.isEmpty())
        throw new ResourceNotFoundException(
            "No tour exists for Rayon " + rayon + " and Year " + year);
      ...
      for (Route route : tour.getRoutes()) {
        ...
        workbook.setSheetName(
            workbook.getSheetIndex(currentGroupSheet),
            route.getGroup().toString() + " Rayon " + "I".repeat(rayon));
        ...
        currentGroupSheet
            ...
            .getCell(5)
            .setCellValue(
                LocalDateTime.from(LocalDateTime.of(tour.getDate(), route.getCustomerStart())));
        currentGroupSheet
            .getRow(7)
            .getCell(1)
            .setCellValue(route.getCustomerStart().toSecondOfDay() / ((double) 3600 * 24));
        int depotToFirstTime =
            route.getCustomerStart().toSecondOfDay()
                - route.getCustomers().get(0).getVisitTime().toSecondOfDay();
        currentGroupSheet
            .getRow(7)
            .getCell(7)
            .setCellValue(((double) Math.abs(depotToFirstTime)) / (3600 * 24));
        ...
        for (int i = 0; i < customers.size(); i++) {
          ...
          currentRow
              .getCell(2)
              .setCellValue(
                  customers.get(i).getLastName().strip()
                      + " "
                      + customers.get(i).getFirstName().strip());
          ...
          if (i + 1 < customers.size()) {
            int duration =
                customers.get(i).getVisitTime().toSecondOfDay()
                    - customers.get(i + 1).getVisitTime().toSecondOfDay();
            currentRow.getCell(7).setCellValue(((double) Math.abs(duration)) / (3600 * 24));
          } else {
            int index = customers.size() - 1;
            int lastToDepot =
                route.getCustomers().get(index).getVisitTime().toSecondOfDay()
                    - route.getCustomerEnd().toSecondOfDay();
