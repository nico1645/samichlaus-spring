...
public class GraphhopperService {
  ...
  private GraphhopperService(YAMLConfig config) {
    this.hopper
        ...
        .setProfiles(
            List.of(
                new Profile("foot").setVehicle("foot").setWeighting("custom").setTurnCosts(false),
  ...
  public double calculateDistance(
    ...
    GHRequest request =
        new GHRequest(fromLat, fromLon, toLat, toLon)
            .setProfile(profile)
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        if (i == 0 && j == 0) {
          ...
          distanceMatrix[i][j] =
              calculateDistance(
                  Constants.DEPOT_LAT_LNG.get(0),
                  Constants.DEPOT_LAT_LNG.get(1),
                  c2.getLatitude(),
                  c2.getLongitude(),
                  "foot");
        } else if (j == 0) {
          ...
          distanceMatrix[i][j] =
              calculateDistance(
                  c1.getLatitude(), c1.getLongitude(), c2.getLatitude(), c2.getLongitude(), "foot");
        }
      }
    }
